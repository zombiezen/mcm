...
import (
	...
	"github.com/zombiezen/mcm/third_party/golang/capnproto/encoding/text"
...
func readTestFile(name string) ([]byte, error) {
	...
	if dir == "" {
		return nil, errors.New("TEST_SRCDIR environment variable empty")
	}
	path := filepath.Join(dir, "com_zombiezen_mcm", "third_party", "golang", "capnproto", "capnpc-go", "testdata", name)
...
func TestRemoteScope(t *testing.T) {
	...
	tests := []scopeTest{
		...
		},
		{
			...
			imports: []importSpec{
				{name: "capnp", path: "zombiezen/third_party/golang/capnproto"},
			},
		},
	}
