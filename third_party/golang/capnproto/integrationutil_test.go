...
const schemaPath = "com_zombiezen_mcm/third_party/golang/capnproto/internal/aircraftlib/aircraft.capnp"

func runfile(name string) string {
	dir := os.Getenv("TEST_SRCDIR")
	if dir == "" {
		panic("TEST_SRCDIR empty")
	}
...
func encodeTestMessage(typ string, text string, fallback []byte) ([]byte, error) {
	...
	b, err := tool.Encode(capnptool.Type{SchemaPath: runfile(schemaPath), Name: typ}, text)
	if err != nil {
